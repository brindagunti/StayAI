...
SERPER_API_KEY = "e07e8348b77af69a52dbe196c8603d7c46dfb169"
llm = GroqInference()
...
class ThinkingTool(Tool):
    ...
    def generate_queries(self,query)->None:
        THINKING_PROMPT="""
        You are an expert planner,who knows how to break down a complex task into smaller,more manageable tasks.
        Your main task is to break down the user query and plan out smaller queries made in such a way that they can be used to search for information in the web.
        ...
        3.Only response in JSON format.
        """
        response = llm.generate_response(messages=[{"role":"system","content":THINKING_PROMPT},{"role":"user","content":SYSTEM_PROMPT},{"role":"user","content":query}])
        processed_response = pre_process_the_json_response(response)
        response_object = load_object_from_string(processed_response)
        return f"Observations from the thinking tool:\n{json.dumps(response_object, indent=2)}\n"
